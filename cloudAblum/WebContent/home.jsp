...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		$(function() {
			...
			$("#accordion").accordion({
				...
				activate:function(event, ui) {
					if (ui.newHeader[0].id == 'header_viewphotos') {
						loadPhotos();
					} else if (ui.newHeader[0].id == 'header_addfriends') {
						searchFriends();
					}
				}
			}).sortable({
		...
		function addToCategory() {
			...
			var toggles = ""; 
			$("input[type='checkbox']:checked", "#tbody_files").each(function(index, element) {
		...
		function sharePhotos() {
			//get photos' toggle ids
			var toggles = ""; 
			$("input[type='checkbox']:checked", "#tbody_photos").each(function(index, element) {
				toggles = toggles + (index == 0 ? "":",") + element.getAttribute("data-id");
			});
			//check photos selected
			if (toggles == "") {
		...
		function loadPhotos() {
			$.ajax({
				...
				success: function (files) {
		        	...
		        	$("#tbody_photos").html(tmpl("template-view-photo", data));
		        	$("tr").addClass("in");
		        	$(".delete", "#tbody_photos").click(function(event) {
		        		var button = event.target;
		        		$.ajax({
		        	...
		        	$("#selectAllPhotos").prop("checked", false);
		        }
			});
		}		
		function deletePhotos() {
			...
			$("input[type='checkbox']:checked", "#tbody_photos").each(function(index, element) {
				//toggles = toggles + (index == 0 ? "":",") + element.getAttribute("data-id");
				$(".delete[data-id="+ element.getAttribute("data-id") +"]").trigger("click");
			});
		}
		function selectAllPhotos() {
			var checkStatus = $("#selectAllPhotos").prop("checked");
			$("input[type='checkbox']", "#tbody_photos").prop("checked", checkStatus);
		}
</script>
	</head>
	<body>
					...
					<div class="mainbar">
						<div class="article">
							<div id="accordion" style="height:600px">
								<div class="group">
									<h3 id="header_addfriends">Add friends</h3>
									<div>
								...
								<div class="group">
									<h3 id="header_addphotos">Add photos</h3>
									<div>
								...
								<div class="group">
									<h3 id="header_viewphotos">Share/view photos</h3>
									<div>
										<div id="categories-photos">
											<button type="button" class="btn delete" onclick="deletePhotos();">
							                    <i class="glyphicon glyphicon-trash"></i>
							                    <span>Delete</span>
							                </button>
							                ...
							                <input type="checkbox" id="selectAllPhotos" onclick="selectAllPhotos();"/>
											<div class="popup-gallery">
												<table role="presentation" class="table table-striped"><tbody id="tbody_photos" class="files"></tbody></table>
											</div>
											<script>
											        ...
											        $('.popup-gallery').magnificPopup({
											            ...
											            image: {
											                ...
											                titleSrc: function(item) {
											                    return item.el.attr('title') + '&nbsp;&nbsp; [' + item.el.attr('data-comment') + ']';
											                }
											            }
											        });
											    });
	...
	<div class="footer">
	  <div class="footer_resize">
		...
		<%@ include file="templates/viewPhoto.html"%>
